...
In each case, the asymptotic score methods provide a corresponding hypothesis test against any specified null parameter value, for a superiority test or a non-inferiority test, with guaranteed coherence between the test and interval. The superiority test is a variant of (and in many cases identical to) a chi-squared test or CMH test, and the non-inferiority test is analogous to a Farrington-Manning test, all with improved control of Type 1 error achieved by the bias and skewness corrections. For paired proportions, the superiority test is a variant of the McNemar test, incorporating an 'N-1' adjustment which appears to avoid any violations of the nominal significance level.
