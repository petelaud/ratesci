...
if (FALSE) {
  diffBinconf.meta <- function(x1, n1, x2, n2, level = 0.95, weight = "MH",
                               incr = 0.5, addincr = F, MH.exact = T, hakn = F,
                               contrast = "RD", distrib = "bin") {
    if (hakn == T) {
      random <- T
      weight <- "Inverse"
    }
    if (distrib == "bin") {
      meta <- metabin(event.e = x1, n.e = n1, event.c = x2, n.c = n2,
                      sm = contrast, backtransf = T, level = level,
                      level.comb = level, method = weight, warn = T,
                      addincr = addincr, incr = incr, MH.exact = MH.exact,
                      hakn = hakn, allstudies = F)
    } else if (distrib == "poi") {
      meta <- metainc(event.e = x1, time.e = n1, event.c = x2, time.c = n2,
                      sm = paste("I", contrast, sep = ""), level = level,
                      level.comb = level, method = weight, warn = T,
                      addincr = addincr, incr = incr, hakn = hakn)
    }
    Z <- qnorm(1 - (1 - level) / 2)
    theta <- meta$TE.fixed
    lcl <- meta$lower.fixed # meta$TE.fixed-Z*meta$seTE.fixed
    ucl <- meta$upper.fixed # meta$TE.fixed+Z*meta$seTE.fixed
    if (!is.na(meta$lower.random)) {
      lcl.rand <- meta$lower.random
    } else lcl.rand <- lcl
      # meta$TE.random - Z*meta$seTE.random
    if (!is.na(meta$TE.random)) {
      theta.rand <- meta$TE.random
    } else theta.rand <- theta
      # meta$TE.random - Z*meta$seTE.random
    if (!is.na(meta$upper.random)) {
      ucl.rand <- meta$upper.random
    } else ucl.rand <- ucl
      # meta$TE.random + Z*meta$seTE.random
    ...
    return(list(estimates = estimates,
                homog = cbind(Q = meta$Q, tau2 = meta$tau^2,
                              I2 = 100 * meta$I2,
                              pvalQ = 1 - pchisq(meta$Q, meta$df.Q))))
  }
}

###################
# Table 2: Example unstratified confidence intervals using
#          cisapride data plus one from Newcombe
###################
# fround <- function(x,digits=6) {
#   paste("(",paste(format(round(x,digits=digits),nsmall=digits),collapse=", "),")",sep="")
# }
fround <- function(x, digits = 6) {
  t(c(round(x, digits = digits)))
}
...
for (i in 1:3) {
  ...
  mytab <- rbind(
    SCAS =
      cbind(
        RDbin = fround(scoreci(x1 = x1, x2 = x2, n1 = n1, n2 = n2,
                               stratified = F, theta0 = 0.5, skew = T,
                               contrast = "RD")$estimates[, c(1, 3)], 3),
        RDpoi = fround(scoreci(x1 = x1, x2 = x2, n1 = n1, n2 = n2,
                               stratified = F, theta0 = 0.5, skew = T,
                               contrast = "RD",
...
if (FALSE) {
  ...
  ###################
  fround <- function(x, digits = 6) {
    paste("(", paste(format(round(x, digits = digits),
                            nsmall = digits), collapse = ", "), ")", sep = "")
  }
  tabS1 <- list()
  for (i in 1:3) {
    x1 <- x1hkp[i]
    ...
    mytab <- rbind(
      SCAScc0.5 =
        c(
          RDbin = fround(scoreci(x1 = x1, x2 = x2, n1 = n1, n2 = n2,
                                 stratified = F, skew = T, contrast = "RD",
          ...
          RRpoi = fround(scoreci(x1 = x1, x2 = x2, n1 = n1, n2 = n2,
                                 ...
                                 distrib = "poi", cc = 0.5)$estimates[, c(1, 3)], 3),
          OR = fround(scoreci(x1 = x1, x2 = x2, n1 = n1, n2 = n2,
                              stratified = F, skew = T, contrast = "OR",
                              cc = 0.5)$estimates[, c(1, 3)], 3)
        ),
      SCAScc0.25 =
        c(
  ...
  ###################
  fround <- function(x, digits = 6) {
    paste("(", paste(format(round(x, digits = digits), nsmall = digits),
                     collapse = ", "), ")", sep = "")
  }
  ...
  tabS2 <- rbind(
    SCAS =
      c(
        pbin1 = fround(scoreci(x1 = x1g, n1 = n1g,
                               contrast = "p")$estimates[, c(1, 3)], 3),
        ppoi1 = fround(scoreci(x1 = x1g, n1 = n1g, contrast = "p",
                               distrib = "poi")$estimates[, c(1, 3)], 3),
        pbin2 = fround(scoreci(x1 = x2g, n1 = n2g,
                               contrast = "p")$estimates[, c(1, 3)], 3),
        ppoi2 = fround(scoreci(x1 = x2g, n1 = n2g,
                               contrast = "p", distrib = "poi")$estimates[, c(1, 3)], 3)
      ),
    Jeffreys =
      c(
        pbin1 = fround(jeffreysci(x = x1g, n = n1g, adj = F)[, c(1, 2)], 3),
        ppoi1 = fround(jeffreysci(x = x1g, n = n1g, adj = F,
                                  distrib = "poi")[, c(1, 2)], 3),
        pbin2 = fround(jeffreysci(x = x2g, n = n2g, adj = F)[, c(1, 2)], 3),
        ppoi2 = fround(jeffreysci(x = x2g, n = n2g, adj = F,
                                  distrib = "poi")[, c(1, 2)], 3)
      ),
    Score =
      c(
        pbin1 = fround(scoreci(x1 = x1g, n1 = n1g, contrast = "p",
                               skew = F)$estimates[, c(1, 3)], 3),
        ppoi1 = fround(scoreci(x1 = x1g, n1 = n1g, contrast = "p",
                               skew = F, distrib = "poi")$estimates[, c(1, 3)], 3),
        pbin2 = fround(scoreci(x1 = x2g, n1 = n2g, contrast = "p",
                               skew = F)$estimates[, c(1, 3)], 3),
        ppoi2 = fround(scoreci(x1 = x2g, n1 = n2g, contrast = "p",
                               skew = F, distrib = "poi")$estimates[, c(1, 3)], 3)
      ),
    Wald =
      c(
