...
- New function
  ...
  for CI and test for a single binomial proportion from clustered data.
...
- `cc` uses a new form of correction for RR giving equivariant
  intervals. Also allows consistency with the continuity-corrected
  McNemar test (or an intermediate correction of the user’s choosing).
  `cctype` is deprecated.
- Default conditional odds ratio method changed to SCASp (with
  closed-form calculation).
- Output object now includes estimates of p1, p2, phi (correlation) and
  psi (odds ratio used by Fagerland et al).
- Output object now includes function call.

#### In `scaspci()`:

- `bcf` option now implemented for contrast = “p” (default = FALSE).
- `bign` allows a different sample size to be used in the bias
  correction (used within transformed SCASp method for paired OR in
...
- ORbias, RRtang, and MNtol arguments renamed as or_bias, rr_tang and
  mn_tol.
- Implementation of `precis` argument is improved for RR and OR
  contrasts.
...
- `cc` continuity correction is now available for all methods for all
  contrasts.
- `cctype` controls the type of correction to apply for `contrast` =
  “RR”.
- New default `method_RD` = “Score_closed” for non-iterative calculation
  of the Tango score interval for `contrast` = “RD”. Thanks to Tony Yang
  for permission to use the code in his 2013 paper.
- New default `method_RR` = “Score_closed” for non-iterative calculation
  of the Tang score interval for `contrast` = “RR”. Thanks to Guogen
  Shan for contributing code via email.
- Added paired MOVER methods with `method_RD` = “MOVER” and `method_RR`
  = “MOVER”. Also “MOVER_newc” incorporates Newcombe’s correlation
  correction.
- Added `moverbase`, for specifying different versions of the MOVER
  methods (Wilson, Jeffreys, midp or SCAS).
...
- Improved handling of special cases for MN weighting
  ([\#25](https://github.com/petelaud/ratesci/issues/25), thanks to
  Vincent Jaquet for reporting the issue and proposed solution. Also
  [\#27](https://github.com/petelaud/ratesci/issues/27) for RR, thanks
  to [@lovestat](https://github.com/lovestat).) As a result, double-zero
  strata need not be excluded when weighting = “MN”.
...
- Same correction affects MOVER method for comparison of Poisson rates
  \[i.e. [`moverci()`](https://petelaud.github.io/ratesci/reference/moverci.md)
  with `distrib` = “poi” and `type` = “wilson”\]

### Other

- Improved documentation of hypothesis tests and continuity corrections,
  clarifying links to Chi-squared tests and CMH test with selected
  weights.
...
- MN weighting now iterates to convergence
  ([@jonjvallejo](https://github.com/jonjvallejo),
  [\#20](https://github.com/petelaud/ratesci/issues/20)).
- Added optional prediction interval for random effects method (also in
  [`tdasci()`](https://petelaud.github.io/ratesci/reference/tdasci.md)).
...
- Added RRtang argument to apply Tang’s alternative score for RR
  (recommended for stratified analysis with INV/IVS weights.
  Experimental for Poisson RR). `Stheta = (p1hat - p2hat * theta) / p2d`
  (see Tang 2020)
...
- Fixed bug in
  [`scoreci()`](https://petelaud.github.io/ratesci/reference/scoreci.md)
  for distrib = “poi” and contrast = “p”
  ([\#7](https://github.com/petelaud/ratesci/issues/7)).
...
- Fixed bug in
  [`scoreci()`](https://petelaud.github.io/ratesci/reference/scoreci.md)
  for stratified zero scores calculated as NA, resulting in UL = 0.
  (Thanks to Lidia Mukina for reporting the bug.)
...
- Added score methods (Tango & Tang) as default for paired binomial RD
  and RR in
  [`pairbinci()`](https://petelaud.github.io/ratesci/reference/pairbinci.md).
...
- Changed point estimate used in
  [`moverci()`](https://petelaud.github.io/ratesci/reference/moverci.md)
  to posterior median for type = “jeff”, to ensure consistent
