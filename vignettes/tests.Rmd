...
The direct equivalence with the Chi-squared test also holds when the skewness correction is included, if group sizes are equal (see below). If group sizes are unequal, then the SCAS 2-sided test is an improved variant of the Chi-squared test.
...
In addition, a pair of 1-sided tests are conducted against a user-specified value of the contrast parameter, $\theta_0$, to cater for non-zero or non-unity null hypotheses for equivalence or non-inferiority tests. The 1-sided test is analogous to the Farrington-Manning test, but not identical to it, due to the 'N-1' variance bias correction and skewness correction, both of which provide improved control of Type 1 error [@laud2017], also [@laud2014], which used the label GNbc for SCAS (Note the Type 1 error for the Farrington-Manning test is the same as 1-sided coverage of the Mee method).
...
The 1-sided tests are a skewness-corrected and 'N-1' adjusted version of Nam's score test [@nam1997], the coverage properties of which are examined in [Laud2025, under evaluation].
