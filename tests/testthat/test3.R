...
test_that("legacy & new methods match published examples", {
  ...
  # Miettinen-Nurminen
  expect_equal(
    unname(round(scoreci(x1 = 5, x2 = 0, n1 = 56, n2 = 29,
                         skew = F)$estimates[, c(1, 3)], 4)),
    c(-0.0326, 0.1933)
  )
  # Mee
  expect_equal(
    unname(round(scoreci(x1 = 5, x2 = 0, n1 = 56, n2 = 29,
                         skew = F, bcf = F)$estimates[, c(1, 3)], 4)),
    c(-0.0313, 0.1926)
  )
  ...
  expect_equal(
    unname(round(scoreci(x1 = 7, x2 = 2, n1 = 25, n2 = 25,
                         bcf = F, skew = F)$estimates[, c(1, 3)], 3)),
    c(-0.014, 0.412)
  )
  expect_equal(
    unname(round(scoreci(x1 = 7, x2 = 2, n1 = 25, n2 = 25,
                         bcf = F, skew = T)$estimates[, c(1, 3)], 3)),
    c(-0.014, 0.414)
  )
  ...
  n0 <- n2 <- c(453, 174, 70)
  event <- c(
    rep(1, x1[1]), rep(0, n1[1] - x1[1]),
    rep(1, x1[2]), rep(0, n1[2] - x1[2]),
    rep(1, x1[3]), rep(0, n1[3] - x1[3]),
    rep(1, x0[1]), rep(0, n0[1] - x0[1]),
    rep(1, x0[2]), rep(0, n0[2] - x0[2]),
    rep(1, x0[3]), rep(0, n0[3] - x0[3])
  )
  strat <- factor(c(rep(1, n1[1]), rep(2, n1[2]),
                    rep(3, n1[3]), rep(1, n0[1]),
                    rep(2, n0[2]), rep(3, n0[3])))
  trt <- factor(c(rep(1, sum(n1)), rep(0, sum(n0))))

  expect_equal(
    scoreci(x1, n1, x2, n2, stratified = T, contrast = "RD", skew = F,
            bcf = T, weighting = "MH", random = F)$pval[2],
    mantelhaen.test(x = event, y = trt, z = strat, correct = F)$p.value
  )
  expect_equal(
    scoreci(x1, n1, x2, n2, stratified = T, contrast = "RR", skew = F,
            bcf = T, weighting = "MH", random = F)$pval[2],
    mantelhaen.test(x = event, y = trt, z = strat, correct = F)$p.value
  )
  expect_equal(
    scoreci(x1, n1, x2, n2, stratified = T, contrast = "OR", skew = F,
            bcf = T, weighting = "INV", random = F)$pval[2],
    mantelhaen.test(x = event, y = trt, z = strat, correct = F)$p.value
  )
  ...
  # Gart
  expect_equal(
    unname(round(scoreci(x1 = 28, n1 = 99,
                         x2 = 23, n2 = 182, contrast = "OR",
                         bcf = F, skew = T, cc = T)$estimates[, c(1, 3)], 2)),
    c(1.40, 5.33)
  )
  # Cornfield
  expect_equal(
  ...
  expect_equal(
    unname(round(scoreci(x1 = c(4, 2, 4, 1), n1 = c(16, 16, 18, 15),
                         x2 = c(5, 3, 10, 3), n2 = c(79, 87, 90, 82),
                         contrast = "OR", bcf = F, skew = T, cc = T,
                         stratified = T)$estimates[, c(1, 3)], 2)),
    c(1.24, 7.12)
  )
  expect_equal(
    unname(round(scoreci(
  ...
  expect_equal(
    unname(round(exactci(x = c(15, 0, 1), n = c(148, 20, 29),
                         distrib = "bin", midp = TRUE)[, c(1, 3)], 4)),
    matrix(c(0.0601, 0.1581, 0, 0.1391, 0.0017, 0.1585), byrow = T, nrow = 3)
  )

  expect_equal(
    unname(round(exactci(x = c(15, 0, 1), n = c(148, 20, 29),
                         distrib = "bin", midp = FALSE)[, c(1, 3)], 4)),
    matrix(c(0.0578, 0.1617, 0, 0.1684, 0.0009, 0.1776), byrow = T, nrow = 3)
  )

  expect_equal(
    unname(round(wilsonci(x = c(15, 0, 1), n = c(148, 20, 29),
                          distrib = "bin", cc = FALSE)[, c(1, 3)], 4)),
    matrix(c(0.0624, 0.1605, 0, 0.1611, 0.0061, 0.1718), byrow = T, nrow = 3)
  )

  expect_equal(
    unname(round(wilsonci(x = c(15, 0, 1), n = c(148, 20, 29),
                          distrib = "bin", cc = 0.5)[, c(1, 3)], 4)),
  ...
  # RD: Tango 1998 Table VII
  # closed form
  expect_equal(
    unname(round(scorepairci(x = c(4, 9, 3, 16), contrast = "RD",
                           closedform = TRUE, skew = FALSE,
  ...
  # closed form
  expect_equal(
    unname(round(scorepairci(x = c(43, 0, 1, 0), contrast = "RD",
                           closedform = TRUE, skew = FALSE, bcf = FALSE,
                           level = 0.90)$estimates[, c(1, 3)], 3)),
    c(-0.096, 0.037)
  )
  # iterative
  expect_equal(
    unname(round(scorepairci(x = c(43, 0, 1, 0), contrast = "RD",
                           skew = FALSE, bcf = FALSE,
                           level = 0.90)$estimates[, c(1, 3)], 3)),
    c(-0.096, 0.037)
  )

  # RR: Tang(NS) 2003 Table II
  # closed form
  expect_equal(
    unname(round(scorepairci(x = c(446, 5, 16, 690), contrast = "RR",
                           closedform = TRUE, skew = FALSE, bcf = FALSE,
                           level = 0.9)$estimates[, c(1, 3)], 3)),
    c(0.958, 0.992) # Tang et al appear to have rounded down to 0.991
  )
  # iterative
  expect_equal(
    unname(round(scorepairci(x = c(446, 5, 16, 690), contrast = "RR",
  ...
  # closed form
  expect_equal(
    unname(round(scorepairci(x = c(43, 0, 1, 0), contrast = "RR",
                           closedform = TRUE, skew = FALSE, bcf = FALSE,
                           level = 0.9)$estimates[, c(1, 3)], 3)),
    c(0.904, 1.039) # Tang et al appear to have rounded down to 1.038
  )
  # iterative
  expect_equal(
    unname(round(scorepairci(x = c(43, 0, 1, 0), contrast = "RR",
  ...
  expect_equal(
    unname(round(scorepairci(x = c(53, 16, 8, 9), contrast = "RD",
                           closedform = TRUE, skew = FALSE,
                           bcf = FALSE)$estimates[, c(1, 3)], 3)),
    c(-0.020, 0.207)
  )
  ...
  expect_equal(
    unname(round(scorepairci(x = c(53, 16, 8, 9), contrast = "OR",
                           bcf = FALSE)$estimates[, c(1, 3)], 4)),
    c(0.8718, 4.8816)
  )
  ...
  # Tango RD
  expect_equal(
    unname(round(scorepairci(x = c(1, 1, 7, 12), contrast = "RD",
                           closedform = TRUE, skew = FALSE,
                           bcf = FALSE)$estimates[, c(1, 3)], 3)),
    c(-0.517, -0.026)
  )
  ...
  expect_equal(
    unname(round(moverpairci(x = c(1, 1, 7, 12), contrast = "RD",
                           corc = TRUE,
                           type = "wilson")$estimates[, c(1, 3)], 3)),
    c(-0.507, -0.026)
  )
  # Wald RD
  expect_equal(
    unname(round(rdpairci(x = c(1, 1, 7, 12))$estimates["Wald", c(1, 3)], 3)),
    c(-0.520, -0.052)
  )
  # Wald-cc RD
  expect_equal(
  ...
  # Bonett-Price RD
  expect_equal(
    unname(round(rdpairci(x = c(1, 1, 7, 12))$estimates["Bonett-Price", c(1, 3)], 3)),
    c(-0.508, -0.013)
  )
  # Wald RR
  expect_equal(
    unname(round(rrpairci(x = c(1, 1, 7, 12))$estimates["Wald", c(1, 3)], 3)),
    c(0.063, 1.000)
  )
  # Bonett-Price RR
  expect_equal(
  ...
  expect_equal(
    unname(round(moverpairci(x = c(20, 12, 2, 16), contrast = "RD", type = "wilson", corc = FALSE)$estimates[, c(1, 3)], 4)),
    c(0.0618, 0.3242)
  )
  # and against Newcombe's method 10 result
  expect_equal(
    unname(round(moverpairci(x = c(20, 12, 2, 16), contrast = "RD", type = "wilson", corc = TRUE)$estimates[, c(1, 3)], 4)),
    c(0.0562, 0.3292)
  )

  # RD: Tang(ML) 2010 example - MOVER-wilson matches:
  expect_equal(
    unname(round(moverpairci(x = c(8, 3, 1, 2), contrast = "RD",
                           corc = TRUE,
                           type = "wilson")$estimates[, c(1, 3)], 4)),
    c(-0.1574, 0.4136)
  )
...
#  )
  ...
  expect_equal(
    unname(round(scorepairci(x = c(8, 3, 1, 2), contrast = "RD",
                           closedform = TRUE, skew = FALSE,
                           bcf = FALSE)$estimates[, c(1, 3)], 4)),
    c(-0.1670, 0.4327)
  )

  # RR: Tang(ML) 2010 - score method doesn't quite match, only to 2dps
  expect_equal(
    unname(round(scorepairci(x = c(8, 3, 1, 2), contrast = "RR",
                           closedform = TRUE, skew = FALSE,
                           bcf = FALSE)$estimates[, c(1, 3)], 4)),
    c(0.776, 2.033)
  )
  ...
  expect_equal(
    unname(round(moverpairci(x = c(8, 3, 1, 2), contrast = "RR", corc = FALSE,
                           type = "wilson")$estimates[, c(1, 3)], 4)),
    c(0.8234, 1.9539)
  )
  # MOVER-jeffreys doesn't match - I've been unable to contact the authors
  ...
  #  expect_equal(
  #    unname(round(moverpairci(x = c(8, 3, 1, 2), contrast = "RR", corc = TRUE,
  #          type = "jeff")$estimates[, c(1, 3)], 4)),
  #    c(0.8101, 2.0236)
  #  )
  ...
  expect_equal(
    unname(round(scorepairci(x = c(1, 1, 7, 12), contrast = "RR",
                           closedform = TRUE, skew = FALSE,
                           bcf = FALSE)$estimates[, c(1, 3)], 4)),
    c(0.0653, 0.9069)
  )
  expect_equal(
    unname(round(moverpairci(x = c(1, 1, 7, 12), contrast = "RR", corc = FALSE,
                           type = "wilson")$estimates[, c(1, 3)], 4)),
    c(0.0686, 0.8695)
  )
  ...
  expect_equal(
    unname(round(scorepairci(x = c(7, 25, 2, 68), contrast = "RD",
                           closedform = TRUE, skew = FALSE,
                           bcf = FALSE)$estimates[, c(1, 3)], 3)),
    c(0.139, 0.321)
  )
  ...
  expect_equal(
    unname(round(moverpairci(x = c(7, 25, 2, 68), contrast = "RD", corc = TRUE,
                           type = "wilson")$estimates[, c(1, 3)], 3)),
    c(0.133, 0.318)
  )
  ...
  # C-P exact
  expect_equal(
    unname(round(orpairci(x = c(1, 1, 7, 12), cc = 0.5)$estimates["Transformed Clopper-Pearson", c(1, 3)], 3)),
    c(0.003, 1.112)
  )
  # Wilson
  expect_equal(
    unname(round(orpairci(x = c(1, 1, 7, 12))$estimates["Transformed Wilson", c(1, 3)], 3)),
    c(0.023, 0.890)
  )
