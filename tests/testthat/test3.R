...
test_that("legacy & new methods match published examples", {
  ...
  # closed form
  expect_equal(
    unname(round(pairbinci(x = c(4, 9, 3, 16), contrast = "RD", method = "Score_closed", skew = FALSE, bcf = FALSE)$estimates[, c(1, 3)], 3)),
    c(-0.027, 0.390)
  )
  # iterative
  expect_equal(
    unname(round(pairbinci(x = c(4, 9, 3, 16), contrast = "RD", method = "Score", skew = FALSE, bcf = FALSE)$estimates[, c(1, 3)], 3)),
    c(-0.027, 0.390)
  )
  # closed form
  expect_equal(
    unname(round(pairbinci(x = c(43, 0, 1, 0), contrast = "RD", method = "Score_closed", skew = FALSE, bcf = FALSE, level = 0.90)$estimates[, c(1, 3)], 3)),
    c(-0.096, 0.037)
  )
  # iterative
  expect_equal(
    unname(round(pairbinci(x = c(43, 0, 1, 0), contrast = "RD", method = "Score", skew = FALSE, bcf = FALSE, level = 0.90)$estimates[, c(1, 3)], 3)),
    c(-0.096, 0.037)
  )
  ...
  # closed form
  expect_equal(
    unname(round(pairbinci(x = c(446, 5, 16, 690), contrast = "RR", method = "Score_closed", skew = FALSE, bcf = FALSE, level = 0.9)$estimates[, c(1, 3)], 3)),
    c(0.958, 0.992) # Tang et al appear to have rounded down
  )
  # iterative
  expect_equal(
    unname(round(pairbinci(x = c(446, 5, 16, 690), contrast = "RR", method = "Score", skew = FALSE, bcf = FALSE, level = 0.9)$estimates[, c(1, 3)], 3)),
    c(0.958, 0.992) # Tang et al appear to have rounded down
  )
  # closed form
  expect_equal(
    unname(round(pairbinci(x = c(43, 0, 1, 0), contrast = "RR", method = "Score_closed", skew = FALSE, bcf = FALSE, level = 0.9)$estimates[, c(1, 3)], 3)),
    c(0.904, 1.039) # Tang et al appear to have rounded down
  )
  # iterative
  expect_equal(
    unname(round(pairbinci(x = c(43, 0, 1, 0), contrast = "RR", method = "Score", skew = FALSE, bcf = FALSE, level = 0.9)$estimates[, c(1, 3)], 3)),
    c(0.904, 1.039) # Tang et al appear to have rounded down
  )

  # paired methods examples from Agresti & Min 2005
  expect_equal(
    unname(round(pairbinci(x = c(53, 16, 8, 9), contrast = "RD", method = "Score_closed", skew = FALSE, bcf = FALSE)$estimates[, c(1, 3)], 3)),
    c(-0.020, 0.207)
  )
  expect_equal(
    unname(round(pairbinci(x = c(53, 16, 8, 9), contrast = "RD", method = "TDAS")$estimates[, c(1, 3)], 4)),
    c(-0.0191, 0.2052)
  )
  expect_equal(
    unname(round(pairbinci(x = c(53, 16, 8, 9), contrast = "RD", method = "Score", skew = FALSE, bcf = FALSE)$estimates[, c(1, 3)], 3)),
    c(-0.020, 0.207)
  )
  expect_equal(
    unname(round(pairbinci(x = c(53, 16, 8, 9), contrast = "OR", method = "SCASp", bcf = FALSE)$estimates[, c(1, 3)], 4)),
    c(0.8718, 4.8816)
  )
  ...
  # Tango RD
  expect_equal(
    unname(round(pairbinci(x = c(1, 1, 7, 12), contrast = "RD", method = "Score_closed", skew = FALSE, bcf = FALSE)$estimates[, c(1, 3)], 3)),
    c(-0.517, -0.026)
  )
  # MOVER Wilson - Fagerland use Newcombe's correlation-corrected 'method 10'
  expect_equal(
    unname(round(pairbinci(x = c(1, 1, 7, 12), contrast = "RD", method = "MOVER_newc", moverbase = "wilson")$estimates[, c(1, 3)], 3)),
    c(-0.507, -0.026)
  )

  # example from Newcombe, against Newcombe's method 8 result
  expect_equal(
    unname(round(pairbinci(x = c(20, 12, 2, 16), contrast = "RD", method = "MOVER", moverbase = "wilson")$estimates[, c(1, 3)], 4)),
    c(0.0618, 0.3242)
  )
  # and against Newcombe's method 10 result
  expect_equal(
    unname(round(pairbinci(x = c(20, 12, 2, 16), contrast = "RD", method = "MOVER_newc", moverbase = "wilson")$estimates[, c(1, 3)], 4)),
    c(0.0562, 0.3292)
  )

  # Tang(ML) RD 2010 example - MOVER-wilson matches, but MOVER-jeffreys doesnt
  expect_equal(
    unname(round(pairbinci(x = c(8, 3, 1, 2), contrast = "RD", method = "MOVER_newc", moverbase = "wilson")$estimates[, c(1, 3)], 4)),
    c(-0.1574, 0.4136)
  )
  # Tang(ML) RD 2010 example - Tang matches
  expect_equal(
    unname(round(pairbinci(x = c(8, 3, 1, 2), contrast = "RD", method = "Score_closed", skew = FALSE, bcf = FALSE)$estimates[, c(1, 3)], 4)),
    c(-0.1670, 0.4327)
  )

  # Tang(ML) RR 2010 - score method doesn't quite match, only to 2dps
  expect_equal(
    unname(round(pairbinci(x = c(8, 3, 1, 2), contrast = "RR", method = "Score_closed", skew = FALSE, bcf = FALSE)$estimates[, c(1, 3)], 4)),
    c(0.776, 2.033)
  )
  # MOVER-wilson matches
  expect_equal(
    unname(round(pairbinci(x = c(8, 3, 1, 2), contrast = "RR", method = "MOVER", moverbase = "wilson")$estimates[, c(1, 3)], 4)),
    c(0.8234, 1.9539)
  )
...
#  expect_equal(
#    unname(round(pairbinci(x = c(8, 3, 1, 2), contrast = "RR", method = "MOVER", moverbase = "jeff")$estimates[, c(1, 3)], 4)),
#    c(0.8101, 2.0236)
#  )

  # DelRocco 2022 RR example
  expect_equal(
    unname(round(pairbinci(x = c(1, 1, 7, 12), contrast = "RR", method = "Score_closed", skew = FALSE, bcf = FALSE)$estimates[, c(1, 3)], 4)),
    c(0.0653, 0.9069)
  )
  expect_equal(
    unname(round(pairbinci(x = c(1, 1, 7, 12), contrast = "RR", method = "MOVER", moverbase = 'wilson')$estimates[, c(1, 3)], 4)),
    c(0.0686, 0.8695)
  )

  # Chang 2024 RD example
  expect_equal(
    unname(round(pairbinci(x = c(7, 25, 2, 68), contrast = "RD", method = "Score_closed", skew = FALSE, bcf = FALSE)$estimates[, c(1, 3)], 3)),
    c(0.139, 0.321)
  )
  ...
  expect_equal(
    unname(round(pairbinci(x = c(7, 25, 2, 68), contrast = "RD", method = "MOVER_newc", moverbase = "wilson")$estimates[, c(1, 3)], 3)),
    c(0.133, 0.318)
  )
  ...
  # midp
  expect_equal(
    unname(round(pairbinci(x = c(1, 1, 7, 12), contrast = "OR", method = "midp")$estimates, 3)[, c(1, 3)]),
    c(0.006, 0.924)
  )
  # C-P exact
  expect_equal(
    unname(round(pairbinci(x = c(1, 1, 7, 12), contrast = "OR", method = "midp", cc=0.5)$estimates, 3)[, c(1, 3)]),
    c(0.003, 1.112)
  )
  # Wilson
  expect_equal(
    unname(round(pairbinci(x = c(1, 1, 7, 12), contrast = "OR", method = "wilson")$estimates, 3)[, c(1, 3)]),
    c(0.023, 0.890)
  )
