...
#'
#' @description
#' Confidence intervals for comparisons of two binomial rates.
#' This convenience wrapper function produces a selection of the methods below
#' as appropriate for the odds ratio
#' contrast (OR), with or without continuity adjustment.
#'
...
orci <- function(x1,
  ...
  if ((any(x1 > n1 + 0.001) || any(x2 > n2 + 0.001))) {
    print("x1 > n1 or x2 > n2 not possible")
    stop()
  }
